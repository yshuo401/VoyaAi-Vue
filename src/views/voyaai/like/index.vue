<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button v-hasPermi="['voyaai:like:remove']" plain type="danger" icon="Delete" :disabled="!selection.length || busy" @click="remove(selection)">批量删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="rows" row-key="id" @selection-change="selection = $event">
      ...
      <el-table-column label="点赞时间" width="170">
        <template #default="{ row }">{{ parseTime(row.createTime) }}</template>
      </el-table-column>
      <el-table-column label="操作" width="140" fixed="right">
        <template #default="{ row }">
          <el-button v-hasPermi="['voyaai:like:query']" link type="primary" @click="showDetail(row)">详情</el-button>
          <el-button v-hasPermi="['voyaai:like:remove']" link type="danger" :disabled="busy" @click="remove([row])">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="detailOpen" title="点赞详情" size="480px">
      <template v-if="detail">
        <el-descriptions :column="1" border class="detail-info">
          <el-descriptions-item label="点赞 ID">{{ detail.id }}</el-descriptions-item>
          ...
          <el-descriptions-item label="点赞时间">{{ parseTime(detail.createTime) }}</el-descriptions-item>
        </el-descriptions>
      </template>
    </el-drawer>
  </div>
...
<script setup name="VoyaAiLike">
import { likeApi as api } from '@/api/voyaai/like'
...
async function loadList() {
  ...
  try {
    ...
    if (version === listVersion) listError.value = '点赞数据加载失败，请检查接口或权限后刷新重试。'
  } finally {
...
async function remove(items) {
  ...
  busy.value = true
  try {
    await proxy.$modal.confirm(`确认删除 ${items.length === 1 ? `「${items[0].targetTitle || '点赞记录'}」` : `选中的 ${items.length} 条点赞记录`}？删除后不可恢复。`)
